...
public  class TradeDoArgs {
    private static Logger logger = LoggerFactory.getLogger("trade");
     public static Map<String, Map<String,String>>  config(){
         ...
         action_code_03.put("name","姓名,1,0,20,^[a-zA-Z\\u4E00-\\u9FA5\\uf900-\\ufa2d·s]{2,20}$");   //X,Y,Z 分别代表 -》X是否必填（0非必填，1必填），Y数据类型（0 不限，1数字，2数字字母）
         ...
         action_code_03.put("cert_order_no","身份证申请号,1,0,50,^[a-zA-Z0-9\\_\\-]{10,50}$");
         ...
         action_code_03.put("lbs","经度纬度,1,0,50,^-?\\d{1,2}\\.\\d{2}/-?\\d{1,3}\\.\\d{2}$");
